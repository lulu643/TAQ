...
from src.TAQQuotesReader import TAQQuotesReader
from src.TAQTradesReader import TAQTradesReader


# Version 1802181651 - See Test_FileManager
def _goodDate(startDate, endDate, dateString):
    try:
        date = int(dateString)
    except:
        return False
...
def _getDates(
    ...
    if startDateString is None:
        startDateString = "20070620"
    if endDateString is None:
    ...
    goodDirs = list(
        filter(
            lambda dirName: _goodDate(startDate, endDate, dirName),
            os.listdir(dir)
        )
    )
    return goodDirs
...
class FileManager(object):
    ...
    def getTradeDates(self, startDateString, endDateString):
        return _getDates(self._baseDir + "trades", startDateString, endDateString)

    def getQuoteDates(self, startDateString, endDateString):
        return _getDates(self._baseDir + "quotes", startDateString, endDateString)
