...
class TAQAdjust:
    ...
    def adjust_trade_price_and_vol(self, ticker, startDateString="20070919", endDateString="20070921"):
        ...
        prices_and_vols=pd.DataFrame(index=range(num_rows), columns=["date","milisecond from midnight","original price","adjusted price","original vol","adjusted vol"])
        ...
        for date in tradeDates:
            ...
            for i in range(trade_file.getN()):
                ...
                adj_price = self.adjust_value(self._adjust_factor_price, datetime.strptime(date, '%Y%m%d').strftime('%Y-%m-%d'), ticker, trade_price)
                prices_and_vols.iloc[sum].loc["adjusted price"]=adj_price
                
                trade_vol=trade_file.getSize(i)
                prices_and_vols.iloc[sum].loc["original vol"]=trade_vol
                adj_vol = self.adjust_value(self._adjust_factor_vol, datetime.strptime(date, '%Y%m%d').strftime('%Y-%m-%d'), ticker, trade_vol)
                prices_and_vols.iloc[sum].loc["adjusted vol"]=adj_vol
                sum+=1
                
        prices_and_vols.to_csv("{}_{}-{}_trades.csv".format(ticker,startDateString,endDateString))
    ...
    def adjust_quote_price_and_vol(self, ticker, startDateString="20070919", endDateString="20070921"):
        ...
        prices_and_vols=pd.DataFrame(index=range(num_rows), columns=["date","milisecond from midnight","adjusted ask price","adjusted ask vol","adjusted bid price","adjusted bid vol"])
        print(prices_and_vols)
        from datetime import datetime
        ...
        for date in tradeDates:
            ...
            for i in range(trade_file.getN()):
                ...
                prices_and_vols.iloc[sum].loc["milisecond from midnight"]=trade_file.getMillisFromMidn(i)
                adj_ask_price = self.adjust_value(self._adjust_factor_price, datetime.strptime(date, '%Y%m%d').strftime('%Y-%m-%d'), ticker, ask_price)
                prices_and_vols.iloc[sum].loc["adjusted ask price"]=adj_ask_price
                adj_bid_price = self.adjust_value(self._adjust_factor_price, datetime.strptime(date, '%Y%m%d').strftime('%Y-%m-%d'), ticker, bid_price)
                prices_and_vols.iloc[sum].loc["adjusted bid price"]=adj_bid_price
                adj_ask_size = self.adjust_value(self._adjust_factor_vol, datetime.strptime(date, '%Y%m%d').strftime('%Y-%m-%d'), ticker, ask_size)
                prices_and_vols.iloc[sum].loc["adjusted ask vol"]=adj_ask_size
                adj_bid_size = self.adjust_value(self._adjust_factor_vol, datetime.strptime(date, '%Y%m%d').strftime('%Y-%m-%d'), ticker, bid_size)
                prices_and_vols.iloc[sum].loc["adjusted bid vol"]=adj_bid_size
                sum+=1
                
        prices_and_vols.to_csv("{}_{}-{}_quotes.csv".format(ticker,startDateString,endDateString))
